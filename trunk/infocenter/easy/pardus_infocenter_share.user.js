...
if (!isset($_SERVER['PHP_AUTH_USER'])) {
	...
	session_start();
	if (
		$_SERVER['PHP_AUTH_USER'] != $_SESSION["account"]->getName() ||
		(
			SettingsMod::USE_ENCRYPTED_PASSWORDS &&
			md5($_SERVER['PHP_AUTH_PW']) != $_SESSION["account"]->getPassword()
		) ||
		(
			!SettingsMod::USE_ENCRYPTED_PASSWORDS &&
			$_SERVER['PHP_AUTH_PW'] != $_SESSION["account"]->getPassword()
		)
	) reject();
