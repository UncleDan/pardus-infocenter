// ==UserScript==
// @name				Pardus Infocenter Share 1.5b2
// @author			Originally by Pio (Orion), updated by Uncledan (Orion)/Larry Legend (Artemis) - Taurvi (Artemis)/Sobkou (Orion) - Aeiri	(Artemis).
// @namespace		pardus.at
// @description		Enables to share combat and hack logs and missions bullettin board. For use with Pardus Infocenter v1.5b2 => 1.5b2.005.
...
function saveCombat() {
	var select = document.getElementById("sltServer");
	var server = servers[select.selectedIndex];
			...
			for (var i = 0; i < resourceCell.childNodes.length; i++) {
				...
				if (tag) {
					...
					if (tag.toLowerCase() == "img") {
						...
						res = {img: name};
						//res = {img_name: name}; //unquote this line and quote the line above if your Infocenter applies the Pio fix for images
					}
				}
				else {
